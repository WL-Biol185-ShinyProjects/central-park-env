...
tail_behavior <- tabItem(tabName = "Tail_Behavior",
                         fluidRow(
                           box(plotOutput("squirrel_plot3")),
                           box(plotOutput("squirrel_plot4"))
                         ),
                         fluidRow(
                           box(
                             title = "Tail Behavior Observations",
                             width = 8,
        ...
        as indifferent to the observer. The image to the right shows a slow flag and a fast twitch.")
                           ),
                           box(
                             title = "",
                             width = 4,
                             img(src = "https://compote.slate.com/images/dc330a74-29be-4dbe-9e9f-e4117064c225.jpg?crop=1180%2C842%2Cx0%2Cy0", width = "100%")
                           )
                         )
)       
activity <- tabItem(tabName = "Activity",
                    fluidRow(
                      box(plotOutput("squirrel_plot7")),
                      box(plotOutput("squirrel_plot8"))
                    ),
                    fluidRow(                                    
                      box(
...
noise <- tabItem(tabName = "Noise",
                 fluidRow(
                   box(plotOutput("squirrel_plot5")),
                   box(plotOutput("squirrel_plot6"))
                 ),
                 fluidRow(
                   box(
        ...
        threats could be the presence of humans in the busy park.",
                       br(), br(),
                       "Please press the noises to hear their sounds."),
                     hr(),
                     fluidRow(
                       column(4,
                              h5("Kuk"),
